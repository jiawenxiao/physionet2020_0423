...
import numpy as np, os, os.path, sys,math
...
def evaluate_12ECG_score(label_directory, output_directory):
    ...
    geometric_mean=math.sqrt(Fbeta_measure*Gbeta_measure)

    return auroc,auprc,accuracy,f_measure,Fbeta_measure,Gbeta_measure,geometric_mean
...
if __name__ == '__main__':
    ...
    auroc,auprc,accuracy,f_measure,f_beta,g_beta,geometric_mean = evaluate_12ECG_score(sys.argv[1], sys.argv[2])

    output_string = 'AUROC|AUPRC|Accuracy|F-measure|Fbeta-measure|Gbeta-measure|geometric_mean \n{:.3f}|{:.3f}|{:.3f}|{:.3f}|{:.3f}|{:.3f}|{:.3f}'.format(auroc,auprc,accuracy,f_measure,f_beta,g_beta,geometric_mean)
    if len(sys.argv) > 3:
